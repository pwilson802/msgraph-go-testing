...
func main() {
	...
	for {
		...
		fmt.Println("4. Testing updating a group")
...
func makeGraphCall(graphHelper *graphhelper.GraphHelper) {
	users, err := graphHelper.ListMembers()
	if err != nil {
		log.Panicf("Error getting users: %v", err)
	}
	for _, user := range users.GetValue() {
...
func updateGroup(graphHelper *graphhelper.GraphHelper) {
	err := graphHelper.UpdateMembers()
	if err != nil {
		log.Panicf("Error updating group: %v", err)
	}
